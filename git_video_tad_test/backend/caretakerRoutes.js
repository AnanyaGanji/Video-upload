...
router.post('/uploadVideo/:childId', auth, upload.single('video'), async (req, res) => {
    ...
    try {
        ...
        await child.save();
        res.status(200).json({ message: "Video uploaded successfully", videoPath: req.file.path });
    } catch (err) {
...
//         // Filter videos by childId
//         const videos = child.videos.filter(video => video.childId.toString() === childId);
...
router.get('/getVideos/:childId', auth, async (req, res) => {
    ...
    try {
        ...
        // Return child's videos
        res.json(child.videos);
    } catch (error) {
        console.error('Error fetching videos:', error);
        res.status(500).json({ message: 'Error fetching videos' });
    }
