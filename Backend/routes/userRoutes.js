...
router.post('/login', async (req, res) => {
  try{
    ...
    if(user.role === 'admin'){
      const centre = centres.find(centre => centre.name === user.name);
      const centreId = centre ? centre.centreId : null;
      return res.header('Authorization', token).send({token : token, centreId: centreId,centres:centres});
    }
    return res.header('Authorization', token).send({token : token, centres : centres});
  }
  catch(err){
